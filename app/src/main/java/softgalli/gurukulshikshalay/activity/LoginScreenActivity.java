...
public class LoginScreenActivity extends AppCompatActivity implements KenBurnsView.TransitionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button sendRequestForDemo = findViewById(R.id.sendRequestForDemo);
        if ((BuildConfig.APPLICATION_ID).equalsIgnoreCase("softgalli.school.mgmt.app")) {
            sendRequestForDemo.setVisibility(View.VISIBLE);
            sendRequestForDemo.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
    ...
    public void loginDialog(final String loginAs) {
        ...
        try {
            ...
            ((Button) dialog.findViewById(R.id.loginButton)).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (Utilz.isOnline(mActivity)) {
                        if (checkValidation()) {
                            dialog.dismiss();
                            userLogin(userId.getText().toString().trim(), password.getText().toString().trim(), loginAs);
                        }
                    } else {
    ...
    public void userLogin(final String userIdStr, final String passwordStr, final String loginAs) {
        Utilz.showDailog(mActivity, mActivity.getResources().getString(R.string.pleasewait));
        retrofitDataProvider.userLogin(mActivity, userIdStr, passwordStr, loginAs, new DownlodableCallback<UserDetailsModel>() {
            @Override
    ...
    private void saveTeacherDetailsLocally(UserDetailsDataModel result, final String loginAs) {
        ...
        ClsGeneral.setPreferences(AppConstants.TOKEN, result.getApiToken());
        ClsGeneral.setPreferences(AppConstants.SCHOOL_ID, result.getSchoolId());
